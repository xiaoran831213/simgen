tst1 <- function(N=5e2, M=2, L=4, a=0, b=1, d=0, e=1, times=1e3, seed=NULL, ...)
{
    arg <- get.arg(skp=c("seed", "times"))
    evt <- arg$evt %||% 1e-8
    psd <- arg$psd %||% evt / 10
    set.seed(seed)
    rhm <- ~G(L) + X(L) + U(2 * L) + D(M)| LD(G, a=8, b=.1) + CX(X, a=1, b=1) + CU(U, a=1, b=1) +
    ...
    for(ii in seq(times))
    {
        ...
        LHS <- list(
            ## `LHS = Y^2`    = .p(Y, 2:2, 1:1),
            ## `LHS = Y:Y`    = .p(Y, 2:2, 2:2),
            `LHS = Y - G` = .p(rG, 2:2, 1:2),
            `LHS = Y - Q` = .p(rQ, 2:2, 1:2),
            `LHS = Y - 0` = Y)
        LHS <- LHS[sapply(LHS, length) > 0]
        MDL <- list(
            `LHS ~ G + X` = lhs ~ {G} + X,
            `LHS ~ Q + X` = lhs ~ {Q} + X)
        CFG <- .d(lhs=names(LHS), mdl=names(MDL)[c(1, 2, 1)])
